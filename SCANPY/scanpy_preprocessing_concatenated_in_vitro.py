...
class Single_Cell_Data_Wrangling(object):
    def __init__(self, cell_path_dict, ensembl2symbol,\
        ...
        if self.load_h5ad:
            ...
            for keys, values in self.cell_dict.items():
                self.concatenated_cell_dict[keys.split('_')[0]].append(values[0])
    ...
    def output_summary_pickle(self):
        print("Exporting summary pickle dictionary.")
        ...
        if self.output_dir:
            ...
            with open('in_vitro_HoC_output_summary.pkl', 'w') as outfile:
